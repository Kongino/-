...
train_dataset_fp = "C:/Users/inoh9/Desktop/ML/featu.txt"
...
for i in range(1, 6):
    for banb in range(1, 50):
        ...
        if (i==1):
            model = tf.keras.Sequential([
                tf.keras.layers.Dense(banb*10, activation=tf.nn.relu, input_shape=(28,)),  # 입력의 형태가 필요합니다.
                tf.keras.layers.Dense(2)
            ])
        if (i==2):
            model = tf.keras.Sequential([
                tf.keras.layers.Dense(banb*10, activation=tf.nn.relu, input_shape=(28,)),  # 입력의 형태가 필요합니다.
                tf.keras.layers.Dense(banb*10, activation=tf.nn.relu, input_shape=(28,)),
                tf.keras.layers.Dense(2)
            ])
        if (i==3):
            model = tf.keras.Sequential([
